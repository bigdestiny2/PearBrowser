...
export interface PearSwarmPeer {
  id: string
  pubkey: string | null
  send(data: Uint8Array | ArrayBuffer | ArrayLike<number>): void
}

export interface PearSwarmChannel {
  channelId: string
  topic: string
  topicHex: string
  protocol: string
  ...
  tier: 'A' | 'B' | 'C'
  peers: PearSwarmPeer[]
  on(event: 'peer', fn: (peer: PearSwarmPeer) => void): void
  on(event: 'message', fn: (peer: PearSwarmPeer, data: Uint8Array) => void): void
  on(event: 'peer-leave', fn: (peer: PearSwarmPeer) => void): void
...
export interface PearSwarmV1API {
  join(topicHex?: string | null, opts?: {
    ...
    appName?: string
    reason?: string
  }): Promise<PearSwarmChannel>
}

export interface PearSwarmAPI {
  v1: PearSwarmV1API
}
