...
export function SiteCard({ name, description, onPress, onAction, actionLabel, size = 'large' }: Props) {
  if (size === 'small') {
    return (
    ...
    <TouchableOpacity onPress={onPress} style={styles.largeCard} activeOpacity={0.7}>
      ...
      {onAction && (
        <TouchableOpacity onPress={onAction} style={styles.actionBtn}>
          <Text style={styles.actionText}>{actionLabel || 'Visit'}</Text>
        </TouchableOpacity>
      )}
    </TouchableOpacity>
  )
...
const styles = StyleSheet.create({
  smallCard: { alignItems: 'center', width: 72, marginRight: 12 },
  smallIcon: {
    width: 48, height: 48, borderRadius: 24,
    backgroundColor: colors.surfaceElevated,
    justifyContent: 'center', alignItems: 'center', marginBottom: 6,
  },
  smallIconText: { color: colors.accent, fontSize: 20, fontWeight: '700' },
  smallName: { color: colors.textPrimary, fontSize: 11, textAlign: 'center' },
  largeCard: {
  ...
  largeIconText: { color: colors.accent, fontSize: 18, fontWeight: '700' },
  largeInfo: { flex: 1 },
  largeName: { color: colors.textPrimary, fontSize: 15, fontWeight: '600' },
  largeDesc: { color: colors.textSecondary, fontSize: 12, marginTop: 2 },
  actionBtn: {
    backgroundColor: colors.surfaceElevated,
